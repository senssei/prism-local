...
- [ ] ~~Telemetry: VRAM used/free in `prism doctor` and a log warning when little is free (`get_gpu_info()` already has `vram_free_mb`).~~ **Promoted → Phase 14.**
...
## Phase 14: VRAM telemetry in `prism doctor` (promoted from Phase 3 backlog §"Small and closed")

> Spec: `spec.md` §4 (P17). Promoted from `plan.md` Phase 3 row "Telemetry: VRAM used/free in `prism doctor` and a log warning when little is free". **Status: shipped on `origin/main` via commit `780c237`; the local copy was subsequently amended to `227f5b2` (`feat: per-device VRAM in prism doctor + low-VRAM warning (Phase 14 / P17)`) but the corrected commit never reached origin because `main` is branch-protected and the force-push was rejected. The diff is identical on both sides; only the subject line on origin is misleading. Operator accepted the situation; future cleanup commit or bypass-protection rebase can reword the subject.** Invariants unchanged (I1–I9 hold). Stdlib-only; hermetic via `patch("prism.cli.get_gpu_info", ...)`. The Phase 3 backlog row "Phase 14: `terminal/*` ..." still uses the label `Phase 14`; that work renumbers to `Phase 15` when it is eventually promoted (deferred — flag for Phase 3 cleanup).

> **Implementation note.** Plan item 14.1's prose mentioned a "today's `❌ GPU:` line" in cmd_doctor — that line did not actually exist before this phase (cmd_doctor printed `❌ NVML Driver: ...` only). To stay consistent with the spec P17 wording (`"the existing ❌ GPU: … branch is unchanged"`), the implementation adds the `❌ GPU: …` block to cmd_doctor at the same time as the VRAM display. cmd_doctor now mirrors cmd_status's NVML-unavailable branch exactly — one extra `print` line, no scope creep. Also: plan item 14.1's fifth test had a typo (`PRISM_VRAM_RESERVE_MB=400` was supposed to produce a warning with `vram_free_mb=500.0`, but 500 < 400 is False); the shipped test uses `PRISM_VRAM_RESERVE_MB=1000` for the warning case and `200` for the no-warning case — same intent, correct numbers.

> **Review fixes (sdlc-review round 1).** 5 findings, all fixed test-first / doc-only:
> 1. **HIGH** — `PRISM_VRAM_RESERVE_MB=inf` crashed the doctor (`OverflowError: cannot convert float infinity to integer` inside `int(threshold)`). `low_vram_threshold_mb()` now rejects non-finite floats (`inf`/`-inf`/`nan`) and falls back to `DEFAULT_VRAM_RESERVE_MB`. New test `test_doctor_does_not_crash_when_PRISM_VRAM_RESERVE_MB_is_inf` covers all three (subtests).
> 2. **MEDIUM** — `int(threshold)` truncated `1536.7` → warning read "1536 MB reserve" but the comparison used 1536.7. Replaced with `%.1f` matching the vram_* convention. New test `test_doctor_warning_message_reports_full_threshold_precision`.
> 3. **LOW** — The plan note claiming `cmd_doctor` "mirrors `cmd_status` exactly" was inaccurate; the `gpu.get('error') or 'Not detected via NVML'` form differed from `cmd_status`'s `gpu.get('error', 'Not detected via NVML')`. Switched cmd_doctor to the same `dict.get(..., default)` form. New test `test_doctor_nvml_unavailable_branch_matches_cmd_status` locks the two together going forward.
> 4. **LOW** — Plan's own risk register called for a one-line note next to the `PRISM_VRAM_RESERVE_MB` env-var row in `docs/getting-started.md`. Added.
> 5. **LOW** — `low_vram_threshold_mb()` docstring silent on non-finite floats. Updated; pairs with finding 1's runtime fix.

> **Commit hygiene follow-up.** Origin carry-forward: the committed subject on origin/main (`780c237 "/fix: update CHANGELOG for Phase 13 findings and enhance error handling in acp.py; add tests for structured tool calls and session cancellation"`) reads as Phase 13 work but actually contains the Phase 14 diff. Acceptable for now per operator; can be cleaned later by (a) a `chore: reword 780c237` no-content amend after a temporary protection bypass, or (b) a rebase with `git rebase -i origin/main --autosquash` once the bypass is granted. No action required today.

> **Numbering.** This phase claims the Phase 14 slot. The Phase 3 backlog row `Phase 14: terminal/* command execution mediated by the ACP client...` (a not-yet-started ACP milestone) retains its prose label in the backlog but will be renumbered to Phase 15 the day it lands. Today's plan.md has no `Phase 14` heading yet, so the slot is free.

- [x] 14.1 `prism/resources.py`: new helper `low_vram_threshold_mb() -> float` reading `$PRISM_VRAM_RESERVE_MB` via the exact same pattern that already lives at `prism/resources.py` `check_can_load` (default `DEFAULT_VRAM_RESERVE_MB` = 1536.0; empty / non-numeric falls back to the default). Export it. `prism/cli.py`: import it next to the existing `inspect_wslconfig, system_memory_info` import. In `cmd_doctor`, between the existing NVML block and the `if OG_AVAILABLE:` block, iterate the same `gpu["devices"]` already fetched on line 61 and print the existing `cmd_status` VRAM line (`f"   • VRAM: {used:.1f} MB used / {total:.1f} MB total ({free:.1f} MB free)"`) per device when NVML is available. For each device with `vram_free_mb < low_vram_threshold_mb()`, emit `getLogger("prism.cli").warning(...)` whose message names `GPU #N`, the live numbers (`vram_used_mb / vram_total_mb / vram_free_mb`), the threshold value (formatted as an integer MB), and the next step (`POST /v1/unload`). The "NVML missing" branch (today's `❌ GPU: …` line) is unchanged and emits no warning. Add a module-level `import logging` and `logger = logging.getLogger("prism.cli")` at the top of `cli.py` (matching the convention used by `prism/mcp.py` and `prism/acp.py`). Tests in `tests/test_prism_cli.py` (new class `TestDoctorVram`, sibling to `TestDoctorWslconfig`):
  - `test_doctor_prints_per_device_vram_when_nvml_available` — patch `prism.cli.get_gpu_info` to return one device with `{index: 0, name: "RTX 5070", compute_capability: "12.0", qualifying_cuda: True, vram_used_mb: 2048.0, vram_total_mb: 12288.0, vram_free_mb: 10240.0}`; run `prism doctor` via `run_cli("doctor")`; assert stdout contains `"VRAM: 2048.0 MB used / 12288.0 MB total (10240.0 MB free)"` and the `prism.cli` logger records no WARNING (assertion: `assertNoLogs("prism.cli", level="WARNING")`; the test must use that or a sibling non-emission assertion).
  - `test_doctor_warns_when_free_vram_below_reserve` — same device fixture but `vram_free_mb: 500.0` and env `PRISM_VRAM_RESERVE_MB=1536`; assert `self.assertLogs("prism.cli", level="WARNING")` succeeds, the captured records contain `"GPU #0"`, `500.0`, `1536`, and the word `"unload"`. Crucially: only one warning record for this scenario.
  - `test_doctor_does_not_warn_when_nvml_unavailable` — patch `get_gpu_info` to return `{"available": False, "devices": [], "error": "NVML: ..."}`; assert stdout contains the existing `❌ GPU: NVML: ...` line and `assertNoLogs("prism.cli", level="WARNING")` passes.
  - `test_doctor_with_two_devices_warns_only_on_low_one` — two devices: device 0 `vram_free_mb: 500.0`, device 1 `vram_free_mb: 8000.0`; assert exactly one WARNING record and its message contains `"GPU #0"` (NOT `"GPU #1"`).
  - `test_doctor_warning_threshold_respects_PRISM_VRAM_RESERVE_MB` — `vram_free_mb: 500.0`; with env `PRISM_VRAM_RESERVE_MB=100` assert no WARNING; with env `PRISM_VRAM_RESERVE_MB=400` assert WARNING. Guards that we actually read the env, not a hard-coded constant.

- [x] 14.2 `docs/cli.md` + `CHANGELOG.md`:
  - `docs/cli.md`: under the existing "`prism doctor`" description (today: line 28-31, summary of NVML / onnxruntime-genai / CUDA provider / Ollama / model conversion / WSL2 `.wslconfig`), replace the prose with a bullet list mirroring `prism status` style, and add a final bullet: "Per-device VRAM usage (used / total / free in MiB). If free VRAM is below `$PRISM_VRAM_RESERVE_MB` (default 1536) on any device, `prism doctor` also emits a `logging.getLogger('prism.cli')` warning naming the device and the next step (`POST /v1/unload`)." `tests/test_docs.py` must continue to pass (the link / mkdocs gate).
  - `CHANGELOG.md`: under `[Unreleased]`, add a `### Added` bullet: "`prism doctor` now reports per-device VRAM usage and emits a `prism.cli` warning when free VRAM is below `$PRISM_VRAM_RESERVE_MB` (default 1536)."
  - Tests: `tests/test_docs.py --only docs` and the changelog gate at `scripts/sdlc_check.py` (no new test for the prose change).

> **Commit note.** Match the established Phases 4–13 pattern: runtime + tests + docs + changelog land in **one** commit. Red step: `python3 scripts/sdlc_check.py --red tests.test_prism_cli.TestDoctorVram.test_doctor_warns_when_free_vram_below_reserve`; the failing reason must be that no `prism.cli` WARNING is emitted today (today `cmd_doctor` doesn't log VRAM at all). Then `test_doctor_prints_per_device_vram_when_nvml_available` must also be red before the fix (the per-device line is absent today).
