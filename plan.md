...
> Spec: `spec.md` §4 (P17). Promoted from `plan.md` Phase 3 row "Telemetry: VRAM used/free in `prism doctor` and a log warning when little is free". **Status: implemented + independently reviewed (5 findings, all fixed); items 14.1–14.2 ticked; gate green (547 tests, exit 0); fixes NOT re-reviewed by a fresh subagent — verified by the same tests the findings were written against. Awaiting operator commit.** Invariants unchanged (I1–I9 hold). Stdlib-only; hermetic via `patch("prism.cli.get_gpu_info", ...)`. The Phase 3 backlog row "Phase 14: `terminal/*` ..." still uses the label `Phase 14`; that work renumbers to `Phase 15` when it is eventually promoted (deferred — flag for Phase 3 cleanup).
